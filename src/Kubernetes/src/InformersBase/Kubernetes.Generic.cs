...
namespace Steeltoe.Kubernetes.InformersBase
{
    public class KubernetesGenericClient : IKubernetesGenericClient
    {
        ...
        public KubernetesGenericClient(IKubernetes client)
        {
            _client = (k8s.Kubernetes)client;
        }
        ...
        {
            var jsonSerializer = JsonSerializer.Create(_client.DeserializationSettings);
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var segments = new List<string> { _client.BaseUri.AbsoluteUri.Trim('/') };
            if (isLegacy)
            {
                segments.Add("api");
            }
            ...
            if (body != null)
            {
                ...
                requestContent = SafeJsonConvert.SerializeObject(body, _client.SerializationSettings);
            ...
            cancellationToken.ThrowIfCancellationRequested();
            var httpResponse = await _client.HttpClient.SendAsync(httpRequest, cancellationToken).ConfigureAwait(false);
            if (shouldTrace)
            {
